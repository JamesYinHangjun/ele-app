                    ...
                    <li class="food-list-hook" v-for="(item,index) in shopInfo.menu" :key="index">
                        ...
                        <div @click="handleFood(food)" class="fooddetails" v-for="(food,i) in item.foods" :key="i">
                            ...
                            <section class="fooddetails-info">
                                <h4>{{food.name}}</h4>
                                <p class="fooddetails-des">{{food.description}}
                                </p>
...
export default {
    ...
    data() {
        return {
            ...
            listHeight: [],     // 12个区列表高度
            selectedFood: null,  // 存储点击的食物
            showFood: false      // 这个是控制食物详情的开关(Food.vue)
        }
    },
    created() {
        this.getData();
    ...
    methods: {
        ...
        selectMenu(index) {
            // console.log(index);   index 是为了找到右侧内容的
            // 这个是右侧商品内容
            let foodlist = this.$refs.foodScroll.getElementsByClassName('food-list-hook');

            let el = foodlist[index];
            console.log(el)
            // 点击左边的图片后，右边滚动到对应元素的位置
