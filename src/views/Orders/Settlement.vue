            ...
            <div>
                ...
                <section class="cart-address">
                    ...
                    <p class="address-detail">
                        ...
                        <span v-else>
                            <!-- 如果myAddress中没有收货地址，就显示"新增收货地址",点击会跳转到addAddress去
                            如果有，就显示选择收货地址,点击跳转到myAddress去 -->
                            <span v-if="havaAddress" @click="$router.push('/myAddress')">选择收货地址</span>
...
export default {
    ...
    data() {
        return {
            havaAddress: false,        // 判断是否有地址
            showTableware: false
        }
    },
    computed: {
        // 这是在MyAddress.vue中将address对象存储到vuex中之后获取的
        userInfo() {
    ...
    methods: {
        ...
        // 一进入这个页面就判断是否有地址. 如果有,就是"选择收货地址"，点击进入myAddress,
        // 不然就是"新增收货地址",点击进入addAddress中
        getData() {
            ...
            .then(res => {
                ...
                if(res.data.myAddress.length > 0) {
                    ...
                    this.havaAddress = false;
                }
            })
        },
        handlePay() {
          // 点击按钮时，先判断是否有收获地址，没有的话，会有提示
          if(!this.userInfo) {
            Toast({
              message: "请选择收货地址",
              position: "bottom",
